...
pub fn load_state() -> Result<Option<ServerProcessState>> {
    let path = state_file_path()?;
    if !path.exists() {
        return Ok(None);
    }

    let data = fs::read_to_string(&path).wrap_err("Failed to read kittynode-server state file")?;
    let state =
        serde_json::from_str(&data).wrap_err("Failed to deserialize kittynode-server state")?;
    Ok(Some(state))
}

pub fn save_state(state: &ServerProcessState) -> Result<()> {
    let path = state_file_path()?;
