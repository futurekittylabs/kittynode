...
pub async fn is_docker_running() -> bool {
    match get_docker_instance().await {
        Ok(_) => true,
        Err(error) => {
            tracing::warn!("Docker not reachable: {}", error);
            false
        }
    }
}
...
pub async fn start_docker() -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    {
        std::process::Command::new("open")
            ...
            .spawn()
            .map_err(|error| {
                error!("Failed to start Docker on macOS: {}", error);
                eyre::eyre!("Failed to start Docker: {error}. Please ensure Docker is installed.")
            })?;
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        for (command, args) in attempts {
            if std::process::Command::new(command)
                ...
                .is_ok()
            {
                info!("Started Docker using: {} {:?}", command, args);
                return Ok(());
            }
        }
        ...
        for path in common_paths {
            ...
            if Path::new(&expanded).exists()
                && std::process::Command::new("cmd")
                    ...
                    .is_ok()
            {
                info!("Started Docker from: {}", expanded);
                return Ok(());
            }
        }
...
async fn get_container_logs_from_docker(
    docker: &Docker,
    container_name: &str,
...
pub async fn get_container_logs(
    ...
    get_container_logs_from_docker(&docker, container_name, tail_lines).await
}
