...
| `kittynode-cli`    | Thin wrapper that forwards subcommands to kittynode-core and can spin up kittynode-server                  |
| `kittynode-server` | Axum HTTP service that exposes the core API for remote management and automation                           |
...
Kittynode has one user facing app: the command line interface (CLI).

The CLI is an imperative shell that invokes the core library. By separating a reusable core, Kittynode is simple, well tested, and extensible to other imperative app shells.
